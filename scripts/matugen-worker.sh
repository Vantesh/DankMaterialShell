...
#   $4 - SYNC_MODE_WITH_PORTAL: Sync mode with portal (unused, kept for compatibility)
#   $5 - Must be "--run" (safety flag)
# Exit codes:
...
validate_arguments() {
  if [[ $# -lt 5 ]]; then
    die 1 "Usage: $0 STATE_DIR SHELL_DIR CONFIG_DIR SYNC_MODE_WITH_PORTAL --run"
  fi

  readonly STATE_DIR="$1"
  readonly SHELL_DIR="$2"
  readonly CONFIG_DIR="$3"
  # $4 SYNC_MODE_WITH_PORTAL is unused, kept for compatibility
  ...
  if [[ "${1:-}" != "--run" ]]; then
    die 1 "Fifth argument must be '--run'"
  fi
}
